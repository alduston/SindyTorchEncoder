...
def train_network(training_data, val_data, params, max_data = 10000):
...
def print_progress(sess, i, loss, losses, train_dict, validation_dict, x_norm, sindy_predict_norm):
    ...
    #print(losses)
    print("Epoch %d" % i)
    coeffs = [1,0,1e-4,1e-5]
    validation_loss_vals = [val * coeff for val, coeff in zip(validation_loss_vals, coeffs)]
    print([f'{Key}: {Loss}' for Key,Loss in zip(list(losses.keys()), validation_loss_vals)])
