...
def BA_test(model_params, training_data, validation_data):
    model_params['sequential_thresholding'] = False
    l = len(training_data['x'])
    train_params = {'bag_epochs': 115, 'pretrain_epochs': 200, 'nbags':  int((1.5 * l) // 250), 'bag_size': 250,
...
def Meta_test(runs = 5, small = False):
    ...
    Meta_A_df.to_csv('../Meta_A_df_N.csv')

    Meta_BA_df = pd.DataFrame.from_dict(Meta_BA_dict, orient='columns')
    Meta_BA_df.to_csv('../Meta_BA_df_N.csv')
...
def run():
    if torch.cuda.is_available():
        ...
        Meta_A_df_N = pd.read_csv('../data/Meta_A_df_N.csv')
        Meta_BA_df_N = pd.read_csv('../data/Meta_BA_df_N.csv')

        plt.plot(Meta_A_df_N['epoch'], Meta_A_df_N[f'active_coeffs_avg'], label='A_test')
        plt.plot(Meta_BA_df_N['epoch'], Meta_BA_df_N[f'active_coeffs_avg'], label='BA_test')
        plt.xlabel('epoch')
        plt.ylabel('# active_coeffs')
        plt.title(f'A v BA avg coeffcount')
        plt.savefig(f'../plots/exp_ncum_avg_N.png')
        torch_training.clear_plt()


        avg_loss_A = np.zeros(len(Meta_A_df_N[f'decoder_{0}']))
        avg_loss_BA = np.zeros(len(Meta_BA_df_N[f'decoder_{0}']))
        for i in [0,1,2,3]:
            plt.plot(Meta_A_df_N['epoch'], Meta_A_df_N[f'active_coeffs_{i}'], label = 'A_test')
            plt.plot(Meta_BA_df_N['epoch'], Meta_BA_df_N[f'active_coeffs_{i}'], label='BA_test')
            plt.legend()
            plt.xlabel('epoch')
            plt.ylabel('# active_coeffs')
            plt.title(f'A v BA coeffcount run {i}')
            plt.savefig(f'../plots/exp_ncum_N{i}.png')

            torch_training.clear_plt()

            Meta_A_df_N[f'avg_loss_{i}'] = Meta_A_df_N[f'decoder_{i}'] + Meta_A_df_N[f'sindy_x_{i}']
            Meta_BA_df_N[f'avg_loss_{i}'] = Meta_BA_df_N[f'decoder_{i}'] + Meta_BA_df_N[f'sindy_x_{i}']

            avg_loss_A += Meta_A_df_N[f'avg_loss_{i}']
            avg_loss_BA += Meta_BA_df_N[f'avg_loss_{i}']

            plt.plot(Meta_A_df_N['epoch'], np.log(Meta_A_df_N[f'avg_loss_{i}']), label='A_test')
            plt.plot(Meta_BA_df_N['epoch'], np.log(Meta_BA_df_N[f'avg_loss_{i}']), label='BA_test')
            plt.legend()
            plt.xlabel('epoch')
            plt.ylabel('Log loss')
            plt.title(f'A v BA decoder loss run {i}')
            plt.savefig(f'../plots/exp_decode_loss_N{i}.png')

            torch_training.clear_plt()

    avg_loss_A  *= .25
    avg_loss_BA *= .25
    plt.plot(Meta_A_df_N['epoch'], np.log(avg_loss_A), label='A_test')
    plt.plot(Meta_BA_df_N['epoch'], np.log(avg_loss_BA), label='BA_test')
    plt.legend()
    plt.xlabel('epoch')
    plt.ylabel('Log loss')
    plt.title(f'A v BA decoder avg loss')
    plt.savefig(f'../plots/exp_avg_decode_loss_N.png')
    torch_training.clear_plt()
