...
def run():
    model_params,training_data, validation_data = get_test_params(4000)

    train_params = {'bag_epochs': 10, 'pretrain_epochs': 100, 'nbags': 100, 'bag_size':7,
                    'subtrain_epochs': 20, 'bag_sub_epochs':5}
    model_params['batch_size'] = 7
    model_params['threshold_frequency'] = 25
    model_params['sequential_thresholding'] = False

    if torch.cuda.is_available():
        train_params = {'bag_epochs': 200, 'pretrain_epochs': 500, 'nbags': 100, 'bag_size': 200,
                        'subtrain_epochs': 40, 'bag_sub_epochs':10}
