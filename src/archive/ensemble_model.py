...
class SindyNetEnsemble(nn.Module):
    def __init__(self, params, device = None):
        ...
        self.sub_model_coeffs = torch.nn.Parameter(torch.stack([submodel['sindy_coeffs'] for submodel in self.submodels]),
                                                   requires_grad= True)
        if self.params['translate']:
            self.sub_model_pre_coeffs = torch.nn.Parameter(torch.stack([submodel['pre_sindy_coeffs']
                                                                        for submodel in self.submodels]), requires_grad=True)
    ...
    def init_submodel(self, idx):
        ...
        if self.params['translate']:
            ...
            translator, translator_layers = None, None
        pre_sindy_coeffs = torch.nn.Parameter(self.init_sindy_coefficients(), requires_grad=True)
        ...
        submodel = {'encoder' : encoder, 'encoder_layers': encoder_layers,
                    'pre_sindy_coeffs': pre_sindy_coeffs, 'sindy_coeffs': sindy_coeffs}
        return submodel
    ...
    def sub_decoder_weights(self, sub_idx):
        decoder_weights = []
        decoder_biases = []
        decoder_layers = self.decoder_layers
        if sub_idx:
            decoder_layers = self.submodels[sub_idx]['translator_layers'] + decoder_layers
    ...
    def encoder_weights(self, bag_idx):
        ...
        encoder_layers.append(self.submodels[bag_idx]['translator_layers'])
    ...
    def masked_predict(self, Theta, coeffs = None):
        masks = [deepcopy(submodel['output_mask']) for submodel in self.submodels]
        if not coeffs:
    ...
    def sindy_pre_predict(self, z, x = None, dx = None):
        Theta = self.Theta(z, x, dx)
        coeffs = self.sub_model_pre_coeffs
        return self.masked_predict(Theta, coeffs)
    ...
    def sub_pre_dx(self, bag_idx, x, dx):
        sub_model = self.submodels[bag_idx]
        z_p = sub_model['encoder'](x)
        Theta = self.Theta(z_p, x, dx)
        coeff_m_p = self.sub_model_pre_coeffs[bag_idx]
        sindy_predict = torch.matmul(Theta, coeff_m_p)
        decoder_weights, decoder_biases = self.sub_decoder_weights(bag_idx)
        activation = self.params['activation']
        sub_dx = z_derivative(z_p, sindy_predict, decoder_weights, decoder_biases, activation=activation)
        return sub_dx


    def masked_pre_dx(self, x, dx):
        sub_models = self.submodels
        masks = self.sub_model_masks()
        pre_dx_stack = torch.stack([self.sub_pre_dx(bag_idx, x, dx).T for bag_idx in range(len(sub_models))])
        pre_dx_masks = torch.stack([self.reshape_mask(mask, pre_dx.shape, first=False)
                                    for mask, pre_dx in zip(masks,  pre_dx_stack)])
        pre_dx = torch.sum(pre_dx_masks * pre_dx_stack, 0).T
        return pre_dx, pre_dx_stack
    ...
    def sub_dx(self, bag_idx, x, dx):
        ...
        activation = self.params['activation']
        sub_dx = z_derivative(z, sindy_predict, decoder_weights, decoder_biases, activation=activation)
        return sub_dx
    ...
    def agr_forward_translate(self, x):
        submodels = self.submodels
        zp_stack = torch.stack([submodel['encoder'](x) for submodel in submodels])
        z_stack = torch.stack([submodel['translator'](zp) for zp, submodel in zip(zp_stack,submodels)])
        z = self.aggregate(z_stack)
        return z, z_stack, zp_stack
    ...
    def masked_forward_translate(self, x):
        submodels = self.submodels
        masks = self.sub_model_masks()
        zp_stack = torch.stack([submodel['encoder'](x) for submodel in submodels])
        z_stack = torch.stack([submodel['translator'](z) for z,submodel in zip(zp_stack,submodels)])
        z_masks = torch.stack([self.reshape_mask(mask, z.shape, first = False) for mask, z in zip(masks, z_stack)])
    ...
    def forward_translate(self, x):
        if self.params['eval']:
            z, z_stack, zp_stack = self.agr_forward_translate(x)
            x_decode = self.decoder(z)
        else:
            z, z_stack, zp_stack = self.masked_forward_translate(x)
            x_decode = self.decoder(z)
        return x_decode, z, z_stack, zp_stack
    ...
    def consistency_loss(self, z, x, dx, ddx = None):
        criterion = nn.MSELoss()
        dx_pre_decode = torch.transpose(self.masked_pre_dx(x, dx)[0],0,1)
        dx_decode = torch.transpose(self.dx_decode(z, x, dx), 0, 1)
        return self.params['loss_weight_consistency'] * criterion(dx_pre_decode, dx_decode)


    def sindy_xpre_loss(self, z, x, dx, ddx = None):
        criterion = nn.MSELoss()
        if self.params['model_order'] == 1:
            dx_pre_decode = torch.transpose(self.masked_pre_dx( x, dx)[0],0,1)
            return self.params['loss_weight_sindy_x_pre'] * criterion(dx , dx_pre_decode)
        else:
            dx_decode, ddx_decode = self.ddx_decode(z, x, dx)
            ddx_decode = torch.transpose(ddx_decode,0,1)
            return  self.params['loss_weight_sindy_x_pre'] * criterion(ddx , ddx_decode)
    ...
    def stage1_Loss(self, x, dx, ddx = None):
        x_decode, z, z_stack, zp_stack = self.forward_translate(x)
        ...
        sindy_xpre_loss = self. sindy_xpre_loss(z, x, dx, ddx)
        consistency_loss = self.consistency_loss(z, x, dx, ddx)
    ...
    def stage2_Loss(self, x, dx, ddx = None):
        x_decode, z, z_stack, zp_stack = self.forward_translate(x)
        decoder_loss = self.decoder_loss(x, x_decode)
        sindy_z_loss = self.sindy_z_loss(z, x, dx, ddx)
        sindy_x_loss = self.sindy_x_loss(z, x, dx, ddx)
