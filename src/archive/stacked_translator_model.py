...
class SindyNetTCompEnsemble(nn.Module):
    def __init__(self, indep_models, device = None):
        ...
        self.Detranslator, self.Detranslator_layers = self.init_detranslator()
    ...
    def get_widths(self, in_dim, out_dim):
        widths = [in_dim]
        while 2*widths[-1] < out_dim :
    ...
    def init_detranslator(self):
        activation_function = self.activation_f
        final_dim = self.params['n_encoders'] * self.params['latent_dim']
        input_dim = self.params['latent_dim']
        widths = self.get_widths(input_dim, final_dim)
        layers = []
        ...
        nn.init.constant_(decoder.bias.data, 0)
        layers.append(decoder)
        Detranslator = nn.Sequential(*layers)
        return Detranslator, layers
    ...
    def dx_decode(self, z, dz_pred):
        detranslator = self.Detranslator
        decoder_weights, decoder_biases = self.get_weights(self.params['stacked_decoder_layers'])
        detranslator_weights, detranslator_biases = self.get_weights(self.Detranslator_layers)
        weights = detranslator_weights + decoder_weights
        biases = detranslator_biases + decoder_biases
        activation = self.params['activation']
        #dz_g = residual_z_derivative(z, dz_pred, detranslator_weights, detranslator_biases, activation).T
        #z_g = detranslator['detranslator'](z)
        dx_decode_stack = z_derivative(z, dz_pred, weights, biases, activation).T
        return dx_decode_stack


    def sub_dx_loss(self, x_translate, dx, dz_pred):
        dx_pred_stack = self.dx_decode(x_translate, dz_pred)
        dx_stack = torch.concat([dx for i in range(self.params['n_encoders'])], dim=1)
        criterion = nn.MSELoss()
        loss = self.params['loss_weight_sindy_x'] * (criterion(dx_stack, dx_pred_stack))
        return loss
    ...
    def val_test(self, x, dx, x_translate_stack, agr_key='mean'):
        ...
        agr_x_translate_stack = self.Detranslator(agr_x_translate)
    ...
    def sub_loss(self, x, dx, encode_idx):
        ...
        decoder = self.params['stacked_decoder']

        detranslator = self.Detranslator
        x_decomp_stack = detranslator(x_translate)
        x_decomp_decode_stack = decoder(x_decomp_stack)

        decoder_loss = sum([self.decode_loss(x_decomp_decode, x) for x_decomp_decode
                          in self.split(x_decomp_decode_stack)])/ self.params['n_decoders']
