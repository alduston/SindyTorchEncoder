...
def A_small_test(model_params, training_data, validation_data):
    model_params['sequential_thresholding'] = True
    l = len(training_data['x'])
    train_params = {'bag_epochs': 0, 'pretrain_epochs': 7000, 'nbags': int(1.5 * l // 300), 'bag_size': 300,
...
def run():
    if torch.cuda.is_available():
        Meta_A_df_ST, Meta_BA_df_ST = Meta_test(runs=5, small=False)
        ...
        for i in [0,1,2,3,4]:
            ...
            plt.xlabel('epoch')
            plt.ylabel('Log loss')
            plt.title(f'A v BA avg loss run {i}')
            plt.savefig(f'../plots/exp_decode_loss_ST{i}.png')
    ...
    plt.savefig(f'../plots/exp_avg_decode_loss_ST.png')
    torch_training.clear_plt()
